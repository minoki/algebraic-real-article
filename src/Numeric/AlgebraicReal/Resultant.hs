...
-- | 整域係数多項式の終結式
resultant_d :: (Eq a, IntegralDomain a) => UniPoly a -> UniPoly a -> a
resultant_d f g
  | (f == 0 && degree g == Just 0) || (degree f == Just 0 && g == 0) = 1
  | f == 0 || g == 0 = 0
  | degree' f == 0 = leadingCoefficient f ^ degree' g
  ...
  | r == 0 = 0
  | degree' f >= degree' g, l >= 0 = (-1)^(degree' f * degree' g) * lc_g^l * resultant_d g r
  | degree' f >= degree' g, l < 0  = (-1)^(degree' f * degree' g) * resultant_d g r `divide` lc_g^(-l)
  | otherwise = (-1)^(degree' f * degree' g) * resultant_d g f
  where
    r = pseudoModP f g
    lc_g = leadingCoefficient g
    l = degree' f - degree' r - (degree' f - degree' g + 1) * degree' g

resultant_poly :: (Eq a, IntegralDomain a) => UniPoly (UniPoly a) -> UniPoly (UniPoly a) -> UniPoly a
resultant_poly = resultant_d
